<script>
    ...
    onMount(() => {
      ...
      script.onload = () => {
        ...
        window.liff.init({ liffId: 'YOUR_LIFF_ID' });
      };
    ...
    <div class="max-w-md w-full space-y-8 p-6 bg-white rounded-lg shadow-lg">
      ...
      <form class="space-y-6" on:submit|preventDefault={login}>
        <div>
          ...
          <input type="email" id="email" class="form-input mt-1 block w-full" bind:value={email} required />
        </div>
  
        <div>
          <label for="password" class="block font-medium text-gray-700">Password</label>
          <input type="password" id="password" class="form-input mt-1 block w-full" bind:value={password} required />
        </div>
